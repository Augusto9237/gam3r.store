...
export default function Especificacoes(props: EspecificacoesProps) {
    ...
    return produto ? (
        <div className="flex-1 flex flex-col gap-1">
            <div className="flex mb-3 max-md:text-sm">
            ...
            {produto?.especificacoes &&
                Object.keys(produto.especificacoes)
                    .filter((k) => k !== 'destaque')
                    .map((chave) => (
                        <div key={chave} className="flex gap-1 max-md:text-xs">
                            <span className="p-2 w-1/3 min-w-fit bg-white/5 rounded">{chave}</span>
